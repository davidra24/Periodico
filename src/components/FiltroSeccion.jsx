...
class FiltroSeccion extends Component {
    constructor() {
        super();
        this.state = {
            articulos: []
    ...
    componentDidMount() {
        request
            ...
            .end((err, res) => {
                var z = JSON.parse(res.text);
                    for(var i in z) {
                        for(var j in z[i]) {
                        console.log(z[i].section);
                    }
                }
    ...
    render() {
        var articulos = this.state.articulos.map((articulo, i) => {var base64;
            base64 = articulo.image;
            return (<div id="article" key ={}>                        
                        <div className="row text-center">
                            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                <figure>
                                    <img className="img-responsive center-block " src={"" + base64} width="640" height="480"/>
                                    <a href={'#articulo-completo' + i} className="" data-toggle="modal">
                                        <figcaption>
                                            <h3>{articulo.name}</h3>
                                            <p>{articulo.date}</p>
                                ...
                                {/* Dialogo modal */}
                                <div className="modal fade" id={'articulo-completo' + i}>
